...
#define IS_ALIGNED(p)  ((((unsigned long)(intptr_t)(p)) % ALIGNMENT) == 0)
...
static bool add_range(range_set_t *ranges, char *lo, size_t size,
    ...
    range_t *prev = tree_find_nearest(ranges->lo_tree, (long unsigned)(intptr_t) lo);
    ...
    p->hi = hi;
    p->index = index;
    tree_insert(ranges->lo_tree, (long unsigned)(intptr_t) lo, (void *) p);
    return true;
}
...
static void remove_range(range_set_t *ranges, char *lo)
{
    range_t *p = (range_t *) tree_remove(ranges->lo_tree, (long unsigned)(intptr_t) lo);
    if (!p)
        return;
    range_t *prev = p->prev;
    range_t *next = p->next;
                           ...
                           const char *filename)
{
    ...
    while (fscanf(tracefile, "%s", type) != EOF) {
        switch(type[0]) {
        case 'a':
            ignore += fscanf(tracefile, "%u %zu", &index, &size);
